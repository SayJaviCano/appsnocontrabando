...
<?php
	...
	$sql = "SELECT * FROM analytics_campaign WHERE (DATE(fecha)>='" . date("Y-m-d", $str_fechaInicio) . "' AND DATE(fecha)<='" . date("Y-m-d", $str_fechaFin) . "')";
	if ($server_name!="") { $sql .= " AND server_name='$server_name'"; }
									...
									<div class="col-md-8">
										<table class="table table-striped">
											...
											<?php
												...
												$sql = "SELECT SUM(visitas) as visitas, utm_source, utm_medium, utm_campaign, url, title FROM analytics_campaign WHERE (DATE(fecha)>='" . date("Y-m-d", $str_fechaInicio) . "' AND DATE(fecha)<='" . date("Y-m-d", $str_fechaFin) . "')";
									...
									<div class="col-md-8">
										<table class="table table-striped">
											...
											<?php
												...
												$sql = "SELECT SUM(visitas) as visitas, utm_source, utm_medium, utm_campaign, url, title FROM analytics_campaign_direct WHERE (DATE(fecha)>='" . date("Y-m-d", $str_fechaInicio) . "' AND DATE(fecha)<='" . date("Y-m-d", $str_fechaFin) . "')";
												$sql.= " AND utm_campaign='$utm_campaign'";
												$sql.= " GROUP BY utm_source, utm_medium, url";
	...
	<div class="col-md-6">
		...
		<div class="card">
			<div class="card-body">
				...
				$sql = "SELECT * FROM analytics_compact WHERE (DATE(fecha)>='" . date("Y-m-d", $str_fechaInicio) . "' AND DATE(fecha)<='" . date("Y-m-d", $str_fechaFin) . "')";
				if ($server_name!="") { $sql .= " AND server_name='$server_name'"; }
				$rs = $mysqli->query($sql);
				while ($fila = $rs->fetch_array(MYSQLI_ASSOC)) 
				{
		...
		<div class="card">
			<div class="card-body">
				...
				<?php
					...
					$sql = "SELECT id, fecha, title, url, SUM(visitas) as visitas FROM analytics_paginas_vistas";
			...
			<div class="card-body">
				...
				<div id="pie-denuncias" style="height:300px;"></div>
					...
					<?php
						...
						$sql = "SELECT id, fecha, ccaa, SUM(denuncias) as denuncias FROM analytics_ccaa";
						$sql.= " WHERE (DATE(fecha)>='" . date("Y-m-d", $str_fechaInicio) . "' AND DATE(fecha)<='" . date("Y-m-d", $str_fechaFin) . "')";
