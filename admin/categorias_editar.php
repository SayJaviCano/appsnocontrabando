...
if ($accion=="modificar") {
	...
	if ($num_total_registros>0) {
		$fila = $rs->fetch_assoc();	
		$sql2 = "Update categorias SET slug='$slug', activo=$activo, orden=$orden, nombre='$nombre', meta_title='$meta_title', meta_description='$meta_description'";	
		$sql2 = $sql2 . " WHERE id=" . $id;
	} else {
		$sql2 = "INSERT INTO categorias (slug, activo, orden, nombre, meta_title, meta_description) VALUES ('$slug', $activo, '$orden', '$nombre', '$meta_title', '$meta_description')";     
	}
	$rs2 = $mysqli->query($sql2); 

	getSiteMap();

    ?><script>document.location.href="categorias_listado.php"</script><?php
	die();
}
...
if ($num_cursos > 0) {
	...
	$sql = "SELECT * FROM categorias ORDER BY orden DESC LIMIT 1"; 
	$rs = $mysqli->query($sql); 
    if ($rs->num_rows > 0) {
		$fila = $rs->fetch_assoc();
		$orden = $fila['orden'] + 1;
...
<head>
  ...
  <title></title>
	...
	function f_volver() { 
		location.href="categorias_listado.php";
	}
	function verificar(){
		var ok=true;
		...
		//---------------------------------------------------------------------				
		if (ok){		
			document.form1.action="categorias_editar.php";
						...
						<div class="row">
							<div class="col-md-12">
								...
								<div class="row">
									...
									<div class="col-md-6">
										<label for="slug" class="mr-sm-2">Url</label> <span class="texto_p"><?php echo $dominio . "category/";?></span>
