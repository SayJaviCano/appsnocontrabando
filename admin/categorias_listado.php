...
if ($accion=="activar") {
	$sql2 = "Update categorias SET activo=" . $valor . " WHERE id=" . $id;
...
if ($accion=="modificar") {
	...
	while ($fila = $rs->fetch_array(MYSQLI_ASSOC)){
		...
		$sql2 = "Update categorias SET orden=" .$orden. " Where id=" . $id;
		$rs2 = $mysqli->query($sql2); 
	}
	getSiteMap();
}
if ($accion=="borrar") {
	foreach ($_POST["borrar"] as $key => $value) { 
		...
		$sql = "DELETE FROM contenidos_categorias WHERE id_categoria=$valor";		/*** BORRA RELACCION ARTICULOS **/
		$rs = $mysqli->query($sql);

		$sql = "DELETE FROM categorias WHERE id=$valor";
		$rs = $mysqli->query($sql);	
	} 
	getSiteMap();
}
  ...
  <title></title>
	...
	<!--
		...
		function f_guardar() { 
			...
			document.form1.action="categorias_listado.php";
		...
		function f_borrar() {
			...
			if(conf==true){
				document.form1.action = "categorias_listado.php?accion=borrar";
				document.form1.submit()
			}
		}	

		function f_editar(id) { 
			location.href = "categorias_editar.php?id=" + id;
		}				
	-->
	</script>
</head>
		...
		if ($rs!="") {
			...
			while ($fila = $rs->fetch_array(MYSQLI_ASSOC)){
				...
				<tr scope="row">
					...
					<td align="center"><a href="categorias_listado.php?id=<?php echo $id?>&val=<?php echo $valor?>&accion=activar" style="padding-right:10px;" title="Activo"><img src="images/<?php echo $img?>" border="0"></a></td>
					...
					<td align="center"><a href="<?php echo $dominio . "category/$slug/"?>" class="btn btn-primary" target="_blanck">Ver</a></td>		
