...
function galeriaDialog(editor){
	tmp_editor = editor;
    dialog = editor.openDialog('iframelinkDialog');
};
...
CKEDITOR.plugins.add('galeria',
{
    ...
    {       
       CKEDITOR.dialog.addIframe('iframelinkDialog', 'Gallery',   'file_galeria.php', 900, 700 );

		var cmd = editor.addCommand('galeria', {exec: galeriaDialog});
		//plugin code goes here
        editor.ui.addButton('galeria',
            {
                label: 'Gallery',
                command: 'galeria',
                icon: CKEDITOR.plugins.getPath('galeria') + 'img.png'
            });
    }
});


function poner_valor_galeria(vReturnValue){		
