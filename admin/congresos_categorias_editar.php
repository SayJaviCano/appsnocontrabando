...
if ($accion=="modificar") {
	...
	$sql = "SELECT * FROM congresos_categorias WHERE id=$id"; 
	$rs = $mysqli->query($sql); 
	$num_total_registros = $rs->num_rows; 
	if ($num_total_registros>0) {
		$fila = $rs->fetch_assoc();	
		$sql2 = "Update congresos_categorias SET slug='$slug', activo=$activo, orden=$orden, nombre='$nombre', meta_title='$meta_title', meta_description='$meta_description'";	
		$sql2 = $sql2 . " WHERE id=" . $id;
	} else {
		$sql2 = "INSERT INTO congresos_categorias (slug, activo, orden, nombre, meta_title, meta_description) VALUES ('$slug', $activo, '$orden', '$nombre', '$meta_title', '$meta_description')";     
	}
	$rs2 = $mysqli->query($sql2); 

	getSiteMap();

    ?><script>document.location.href="congresos_categorias_listado.php"</script><?php
	die();
}
...
if ($num_cursos > 0) {
	...
	$sql = "SELECT * FROM congresos_categorias ORDER BY orden DESC LIMIT 1"; 
	$rs = $mysqli->query($sql); 
    if ($rs->num_rows > 0) {
		$fila = $rs->fetch_assoc();
		$orden = $fila['orden'] + 1;
...
$conta_meta_title = $max_title - strlen($meta_title);
if ($conta_meta_title < 0) { $conta_meta_title= '<span class="red">' . $conta_meta_title . '</span>';}

$conta_meta_description = $max_description - strlen($meta_description);
if ($conta_meta_description < 0) { $conta_meta_description= '<span class="red">' . $conta_meta_description . '</span>';}
...
<head>
  ...
  <title></title>
	...
	function carga_categoria_slug(nombre_categoria) {
		$.ajax({
		type: "POST",
		url: 'ajax_congresos_categoria_slug.php', 
	...
	function f_volver() { 
		location.href="congresos_categorias_listado.php";
	}
	function verificar(){
		var ok=true;
		...
		//---------------------------------------------------------------------				
		if (ok){		
			document.form1.action="congresos_categorias_editar.php";
	...
	<div id="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col">
                        <h4 class="page-title">Congresos Categoría</h4>
                    </div>
                </div>
            </div>
						...
						<div class="row">
							<div class="col-md-12">
								...
								<div class="row">
									...
									<div class="col-md-6">
										<label for="slug" class="mr-sm-2">Url</label> <span class="texto_p"><?php echo $dominio . "category/congresos/";?></span>
								...
								<div class="row mt-3">	
									<div class="col-md-12">
										<label for="meta_title" class="mr-sm-2">Meta Title <span id="charNumt" class="contador"><?php echo $conta_meta_title;?></span></label>
									...
									<div class="col-md-12">
										<label for="meta_description" class="mr-sm-2">Meta Description <span id="charNum" class="contador"><?php echo $conta_meta_description;?></span></label>
