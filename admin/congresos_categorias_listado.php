...
if ($accion=="activar") {
	$sql2 = "Update congresos_categorias SET activo=" . $valor . " WHERE id=" . $id;
	$rs2 = $mysqli->query($sql2); 
}
if ($accion=="modificar") {
	$sql = "SELECT * FROM congresos_categorias Order by orden, nombre";
	$rs = $mysqli->query($sql); 
	while ($fila = $rs->fetch_array(MYSQLI_ASSOC)){
		$id = $fila['id'];	
		$orden = $_POST["orden_".$id] ;
		$sql2 = "Update congresos_categorias SET orden=" .$orden. " Where id=" . $id;
		$rs2 = $mysqli->query($sql2); 
	}
	getSiteMap();
}
if ($accion=="borrar") {
	foreach ($_POST["borrar"] as $key => $value) { 
		...
		$sql = "DELETE FROM congresos_categorias WHERE id=$valor";
		$rs = $mysqli->query($sql);	
	} 
	getSiteMap();
}
  ...
  <title></title>
	...
	<!--
		...
		function f_guardar() { 
			...
			document.form1.action="congresos_categorias_listado.php";
		...
		function f_borrar() {
			...
			if(conf==true){
				document.form1.action = "congresos_categorias_listado.php?accion=borrar";
				document.form1.submit()
			}
		}	

		function f_editar(id) { 
			location.href = "congresos_categorias_editar.php?id=" + id;
		}				
	-->
	</script>
</head>
	...
	<div id="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col">
                        <h4 class="page-title">Congresos Categorías</h4>
                    </div>
                </div>
            </div>
		...
		if ($rs!="") {
			...
			while ($fila = $rs->fetch_array(MYSQLI_ASSOC)){
				...
				<tr scope="row">
					...
					<td align="center"><a href="congresos_categorias_listado.php?id=<?php echo $id?>&val=<?php echo $valor?>&accion=activar" style="padding-right:10px;" title="Activo"><img src="images/<?php echo $img?>" border="0"></a></td>
					<td align="center"><input type="text" id="orden_<?php echo $id;?>" name="orden_<?php echo $id;?>" class="form-control" value="<?php echo $orden;?>" maxlength="5"/></td>				
					<td><?php echo "$nombre";?></td>					   
					<td align="center"><a href="<?php echo $dominio . "category/congresos/$slug/"?>" class="btn btn-primary" target="_blanck">Ver</a></td>		
