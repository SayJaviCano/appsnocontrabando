...
if ($accion=="modificar") {
	...
	$sql = "SELECT * FROM contenidos WHERE tipo='page' AND post_name='$slug'"; 
	$rs = $mysqli->query($sql); 
	$num_total_registros = $rs->num_rows; 
	if ($num_total_registros>0) {
		$sql2 = "Update contenidos SET activo=$activo, post_name='$slug', meta_title='$meta_title', meta_description='$meta_description', titulo='$titulo', entradilla='$entradilla', texto='$content'";
		$sql2 = $sql2 . " WHERE post_name='$slug'"; 
	} else {
		$sql2 = "INSERT INTO contenidos (activo, post_name, meta_title, meta_description, titulo, entradilla, texto, tipo) VALUES ($activo, '$slug', '$meta_title', '$meta_description', '$titulo', '$entradilla', '$content', 'page')";     
	}
...
$sql = "SELECT * FROM contenidos WHERE post_name='$slug' AND tipo='page'"; 
$rs = $mysqli->query($sql); 
...
<head>
  ...
  <title></title>
	...
	function verificar(){
		...
		//---------------------------------------------------------------------				
		if (ok){		
			document.form1.action="contenido_editar.php";
