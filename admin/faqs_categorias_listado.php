...
if ($accion=="activar") {
	$sql2 = "Update faqs_categorias SET activo=" . $valor . " WHERE id=" . $id;
...
if ($accion=="modificar") {
	...
	while ($fila = $rs->fetch_array(MYSQLI_ASSOC)){
		...
		$sql2 = "Update faqs_categorias SET orden=" .$orden. " Where id=" . $id;
		$rs2 = $mysqli->query($sql2); 
	}
}
if ($accion=="borrar") {
	foreach ($_POST["borrar"] as $key => $value) { 
		...
		$sql = "DELETE FROM faqs WHERE id_categoria=$valor";		/*** BORRA RELACCION ARTICULOS **/
		$rs = $mysqli->query($sql);

		$sql = "DELETE FROM faqs_categorias WHERE id=$valor";
		$rs = $mysqli->query($sql);	
	} 
}
  ...
  <title></title>
	...
	<!--
		...
		function f_guardar() { 
			...
			document.form1.action="faqs_categorias_listado.php";
		...
		function f_borrar() {
			...
			if(conf==true){
				document.form1.action = "faqs_categorias_listado.php?accion=borrar";
				document.form1.submit()
			}
		}	

		function f_editar(id) { 
			location.href = "faqs_categorias_editar.php?id=" + id;
		}				
	-->
	</script>
</head>
	...
	<div id="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col">
                        <h4 class="page-title">Faqs Categorías</h4>
                    </div>
                </div>
            </div>
		...
		if ($rs!="") {
			...
			while ($fila = $rs->fetch_array(MYSQLI_ASSOC)){
				...
				<tr scope="row">
					...
					<td align="center"><a href="faqs_categorias_listado.php?id=<?php echo $id?>&val=<?php echo $valor?>&accion=activar" style="padding-right:10px;" title="Activo"><img src="images/<?php echo $img?>" border="0"></a></td>
