...
$_SESSION['dir_img_editor'] = "../images/faqs/";
...
if ($accion=="modificar") {
	...
	if ($num_total_registros>0) {
		$sql2 = "Update faqs SET activo=$activo, id_categoria=$id_categoria, orden='$orden', pregunta='$pregunta', respuesta='$respuesta'";
		$sql2 = $sql2 . " WHERE id='$id'"; 
	} else {
		$sql2 = "INSERT INTO faqs (activo, id_categoria, orden, pregunta, respuesta) VALUES ($activo, $id_categoria, '$orden', '$pregunta', '$respuesta')";     
	}
	$rs2 = $mysqli->query($sql2);
	//echo "$sql2<br>";
	?><script>location.href="faqs_listado.php?cat=<?php echo $id_categoria;?>";</script><?php
}
...
if ($rs->num_rows > 0) {
	...
	$sql = "SELECT * FROM faqs WHERE id_categoria=$id_categoria ORDER BY orden DESC LIMIT 1"; 
	$rs = $mysqli->query($sql); 
    if ($rs->num_rows > 0) {
		$fila = $rs->fetch_assoc();
		$orden = $fila['orden'] + 1;
						...
						<div class="row">
							<div class="col-md-12">
								<div class="row">
									...
									<div class="col-md-4">
										<label for="cat" class="mr-sm-2">Categoría</label>
										<select id="cat" name="cat" class="custom-select" title="Selecciona Categoría" data-width="400px" >
											...
											<?php
												...
												while ($fila = $rs->fetch_array(MYSQLI_ASSOC)){
													$activo = "";
													if ($id_categoria == $fila['id']) { $activo = "selected"; }
