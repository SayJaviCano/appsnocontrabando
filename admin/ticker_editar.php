...
if ($accion=="modificar") {
	...
	if ($num_total_registros>0) {
		$fila = $rs->fetch_assoc();	
		$sql2 = "Update ticker SET activo=$activo, orden=$orden, texto='$texto'";	
		$sql2 = $sql2 . " WHERE id=" . $id;
	} else {
		$sql2 = "INSERT INTO ticker (activo, orden, texto) VALUES ($activo, '$orden', '$texto')"; 
	}
	$rs2 = $mysqli->query($sql2); 
    ?><script>document.location.href="ticker_listado.php"</script><?php
}
...
if ($num_cursos > 0) {
	...
	$sql = "SELECT * FROM ticker ORDER BY orden DESC LIMIT 1"; 
	$rs = $mysqli->query($sql); 
    if ($rs->num_rows > 0) {
		$fila = $rs->fetch_assoc();
		$orden = $fila['orden'] + 1;
...
<head>
  ...
  <title></title>
	...
	$(document).ready(function () { 
		CKEDITOR.replace('texto', { height : '200'});

	});

	function f_volver() { 
		location.href="ticker_listado.php";
	}
	function verificar(){
		var ok=true;		
		//---------------------------------------------------------------------				
		if (ok){		
			document.form1.action="ticker_editar.php";
					...
					<!-- CONTENT -->
						...
						<div class="row">
							<div class="col-md-12">
								...
								<div class="row mt-3">	
									<div class="col-md-12">
										<label for="texto" class="mr-sm-2">Ticker</label>
