...
if ($accion=="activar") {
	$sql2 = "Update ticker SET activo=" . $valor . " WHERE id=" . $id;
	$rs2 = $mysqli->query($sql2); 
}
if ($accion=="modificar") {
	$sql = "SELECT * FROM ticker Order by orden, nombre";
	$rs = $mysqli->query($sql); 
	while ($fila = $rs->fetch_array(MYSQLI_ASSOC)){
		$id = $fila['id'];	
		$orden = $_POST["orden_".$id] ;
		$sql2 = "Update ticker SET orden=" .$orden. " Where id=" . $id;
		$rs2 = $mysqli->query($sql2); 
	}
}
...
if ($accion=="borrar") {
	foreach ($_POST["borrar"] as $key => $value) { 
		...
		$sql = "DELETE FROM ticker WHERE id=$valor";
		$rs = $mysqli->query($sql);	
	} 
}
  ...
  <title></title>
	...
	<!--
		...
		function f_guardar() { 
			...
			document.form1.action="ticker_listado.php";
		...
		function f_borrar() {
			...
			if(conf==true){
				document.form1.action = "ticker_listado.php?accion=borrar";
				document.form1.submit()
			}
		}	

		function f_editar(id) { 
			location.href = "ticker_editar.php?id=" + id;
		}				
	-->
	</script>
</head>
...
<form name="form1" id="form1" method="post">
	...
	<?php
		...
		$sql = "SELECT * FROM ticker ORDER BY orden";
		$rs = $mysqli->query($sql); 
		if ($rs!="") {
			...
			while ($fila = $rs->fetch_array(MYSQLI_ASSOC)){
				...
				<tr scope="row">
					...
					<td align="center"><a href="ticker_listado.php?id=<?php echo $id?>&val=<?php echo $valor?>&accion=activar" style="padding-right:10px;" title="Activo"><img src="images/<?php echo $img?>" border="0"></a></td>
