...
if ($page_name=="dispositivo" && $i==1) {
    ...
    if (isset($_POST['token']) && isset($_POST['tipo'])) {
        $token = LimpiaParametros((isset($_POST['token']) ? $_POST['token'] :""));
	...
	$arr = array('id' => $id_dispositivo);
	echo json_encode($arr);
}

elseif ($page_name=="baja" && $i==1) {
	//https://appsnocontrabando.com/api/baja/
	...
	if ($token!="")
	{
		$sql = "SELECT * FROM app_dispositivos WHERE token='$token'"; 
		$rs = $mysqli->query($sql); 
		if ($rs->num_rows > 0) {		
			$fila = $rs->fetch_assoc();
			$id_dispositivo = $fila['id'];
			
			$sql2 = "DELETE FROM app_dispositivos WHERE token='$token' AND id=" . $id_dispositivo;
			$rs2 = $mysqli->query($sql2); 
		}
	}
...
elseif ($page_name=="received" && $i==1) {
	//https://appsnocontrabando.com/api/received/

	$id_dispositivo = 0;
    if (isset($_POST['token']) && isset($_POST['id'])) {
        $token = LimpiaParametros((isset($_POST['token']) ? $_POST['token'] :""));
        $firebase_notificacion_id = LimpiaParametros((isset($_POST['id']) ? $_POST['id'] :""));        
    } else {
        $json = file_get_contents('php://input');
        $obj = json_decode($json, true);	
        $token = $obj['token'];
        $firebase_notificacion_id = $obj['id'];
    }

    if (!is_numeric($firebase_notificacion_id)) { $firebase_notificacion_id = 0; }

    $id_notificacion = 0;
	$id_dispositivo = 0;
	if ($token!="" && $firebase_notificacion_id!=0)
	{		
		$sql = "SELECT * FROM app_dispositivos WHERE token='$token'"; 
		$rs = $mysqli->query($sql); 
        if ($rs->num_rows > 0) {
			...
			$fecha = date("Y-m-d H:i:s");
			$sql4 = "UPDATE app_ca_envios SET fecha_recibido='$fecha', recibido=1 WHERE id_dispositivo=$id_dispositivo AND id_notificacion='$firebase_notificacion_id'";
...
elseif ($page_name=="open" && $i==1) {
	//https://appsnocontrabando.com/api/open/
	...
	{		
        ...
        if ($rs->num_rows > 0) {
			...
			$fecha = date("Y-m-d H:i:s");
            $sql4 = "UPDATE app_ca_envios SET clic=1, fecha_clic='$fecha' WHERE id_dispositivo=$id_dispositivo AND id_notificacion='$firebase_notificacion_id'";
			$rs4 = $mysqli->query($sql4);  			
        }
	}
...
elseif ($page_name=="cookie" && $i==1) {
	//https://appsnocontrabando.com/api/cookie/

    $valor_cookie = "";
    if (isset($_POST['cookie']) ) {
        $valor_cookie = LimpiaParametros((isset($_POST['cookie']) ? $_POST['cookie'] :""));    
    } else {
        $json = file_get_contents('php://input');
        $obj = json_decode($json, true);	
        $valor_cookie = $obj['cookie'];
    }

    if ($valor_cookie=="true")
    {
        setcookie("cookiepolicy-statistics", $valor_cookie, time()+(60*60*24*365), "/");	
    } elseif ($valor_cookie=="false") {
        setcookie("cookiepolicy-statistics", "", time() - 1, "/");
    }
    	
	$arr = array('cookie' => $valor_cookie);
	echo json_encode($arr);
}
