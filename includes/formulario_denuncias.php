...
$(document).ready(function () { 
	$("#comunidad_autonoma").change(function () {
		$("#comunidad_autonoma option:selected").each(function () {
			elegido=$(this).val();			
			$.post("../includes/ajax_combo_provincia.php", { ca: elegido }, function(data){ $("#provincia").html(data);      });     
		});
	});
...
function enviar_datos() {
	...
	if (ok){	
		...
		$.ajax({
			type: "POST",
			url: url, 
			data: {"acc": "denuncia", "comunidad_autonoma": ca,  "provincia": prov,  "localidad": localidad,  "tipo_punto_venta": tipo_punto_venta,  "nombre": nombre,  "telefono_contacto": telefono_contacto,  "direccion": direccion,  "comentarios": comentarios,  "tu_telefono": tu_telefono,  "verificacion": verificacion },
